...
namespace KMCI_System.PurchasingModule.PurchaseRequestModule
{
    public partial class PurchaseRequestDetails : UserControl
    {
        ...
        public PurchaseRequestDetails(string projectCode)
        {
        ...
        {
            LoadUserControl(new PurchaseRequest());
        }
        ...
        private void dgvQuotationItems_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                LoadUserControl(new CreatePurchaseRequest(vendorName, projectCode));
            }
        }
            ...
            try
            {
                ...
                using (MySqlConnection conn = new MySqlConnection(GetConnectionString()))
                {
                    ...
                    string query = @"
                        ...
                        WHERE q.project_code = @project_code AND q.status = 'Approved'
                        GROUP BY qi.pref_vendor
                        ORDER BY qi.pref_vendor";
