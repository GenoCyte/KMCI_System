...
namespace KMCI_System.PurchasingModule.PurchaseRequestModule
{
    public partial class CreatePurchaseRequest : UserControl
    {
        ...
        public CreatePurchaseRequest(string vendorName)
        {
        ...
        public CreatePurchaseRequest(string vendorName, string projectCode)
        {
            this.vendorName = vendorName;
        ...
        private void btnCreatePr_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("No items available to create a purchase request.", "Validation Error",
                ...
                using (MySqlConnection conn = new MySqlConnection(GetConnectionString()))
                {
                    ...
                    try
                    {
                        ...
                        // Validate if purchase request already exists
                        if (IsPurchaseRequestAlreadyCreated(conn, vendorId, quotationId))
                        {
                            string message = quotationId.HasValue
                                ? $"A Purchase Request has already been created for vendor '{vendorName}' with this quotation."
                                : $"A Purchase Request has already been created for vendor '{vendorName}'.";

                            MessageBox.Show(message, "Duplicate Purchase Request",
                                MessageBoxButtons.OK, MessageBoxIcon.Warning);
                            transaction.Rollback();
                            return;
                        }
                        ...
                        // Generate PR Number
                        prNumber = GeneratePRName(conn, transaction);
                        ...
                        string insertQuery = @"
                            INSERT INTO purchase_request (project_code,pr_name, vendor_id, quotation_id, vendor_name, quantity, grand_total, status)
                            VALUES (@project_code, @pr_name, @vendor_id, @quotation_id, @vendor_name, @quantity, @grand_total, @status)";
                        ...
                        using (MySqlCommand cmd = new MySqlCommand(insertQuery, conn, transaction))
                        {
                            ...
                            if (rowsAffected > 0)
                            {
                                purchaseRequestId = (int)cmd.LastInsertedId;

                                // Insert all items into purchase_request_items table
                                string insertItemsQuery = @"
                                    INSERT INTO purchase_request_items (pr_id, sku_upc, prod_name, brand, quantity, unit_price, sub_total)
                                    VALUES (@pr_id, @sku_upc, @prod_name, @brand, @quantity, @unit_price, @sub_total)";
                                ...
                                foreach (DataGridViewRow row in dgvItems.Rows)
                                {
                                    using (MySqlCommand itemCmd = new MySqlCommand(insertItemsQuery, conn, transaction))
                                    {
                                        ...
                                        itemCmd.Parameters.AddWithValue("@pr_id", purchaseRequestId);
                                ...
                                // Export to PDF
                                //ExportToPdf(purchaseRequestId, prNumber);
                                PurchaseRequestPDFGenerator.GeneratePR(
                                    prNumber,
                                    vendorName,
                                    projectCode
                                );

                                MessageBox.Show($"Purchase Request {prNumber} Created Successfully!", "Success",
            ...
            {
                MessageBox.Show($"Error creating Purchase Request: {ex.Message}", "Database Error",
                    MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }

        private string GeneratePRName(MySqlConnection conn, MySqlTransaction transaction)
        {
            try
            {
                string maxQuery;

                // If projectCode is available, generate PR number based on project
                if (!string.IsNullOrEmpty(projectCode))
                {
                    maxQuery = @"
                SELECT MAX(CAST(SUBSTRING(pr_name, +4) AS UNSIGNED)) 
                FROM purchase_request pr
                INNER JOIN quotation q ON pr.quotation_id = q.quotation_id
                WHERE pr.pr_name LIKE 'PR-%'
                    AND q.project_code = @project_code";

                    using (MySqlCommand cmd = new MySqlCommand(maxQuery, conn, transaction))
                    {
                        string projectPrefix = $"{projectCode}-PR";
                        cmd.Parameters.AddWithValue("@project_prefix", projectPrefix);
                        cmd.Parameters.AddWithValue("@project_code", projectCode);

                        object result = cmd.ExecuteScalar();
                        int nextNumber = (result == null || result == DBNull.Value) ? 1 : Convert.ToInt32(result) + 1;

                        // Format: PROJECTCODE-PR-XXX (3-digit zero-padded)
                        string prNumber = $"PR-{nextNumber:D3}";
                        MessageBox.Show($"Generated PR Number: {prNumber}");
                        return prNumber;
                    }
                }
                else
                {
                    // Fallback to original logic when no project code
                    maxQuery = @"
                SELECT MAX(CAST(SUBSTRING(pr_name, 4) AS UNSIGNED)) 
                FROM purchase_request
                WHERE pr_name LIKE 'PR-%' AND vendor_name = @vendor_name";
                    ...
                    using (MySqlCommand cmd = new MySqlCommand(maxQuery, conn, transaction))
                    {
                        ...
                        // Format: PR-XXX (3-digit zero-padded)
                        string prNumber = $"PR-{nextNumber:D3}";
                        MessageBox.Show($"Generated PR Number: {prNumber}");
                        return prNumber;
                    }
            ...
            {
                System.Diagnostics.Debug.WriteLine($"Error generating PR name: {ex.Message}");
        ...
        private void LoadVendorItems()
        {
            try
            {
                ...
                WHERE qi.pref_vendor = @vendor_name
                    ...
                    query += " GROUP BY qi.sku_upc, qi.quantity, qi.unit_price, qi.sub_total, p.prod_name, p.brand";
